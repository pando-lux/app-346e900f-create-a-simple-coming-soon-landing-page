...
Agent ID: worker-devops-b152ff04
Scope: private
Reports to: orch-user_project-1d0a9d56

## Your Role
Deploy the Moonbeam project to live hosting.
...
2. POST to http://127.0.0.1:4100/v1/projects/346e900f13e7b8d14798c54f/validate-deploy with body {"workspaceDir":"C:\\Users\\jaira\\.pando\\projects\\346e900f13e7b8d14798c54f"} and Authorization: Bearer <token>
3. If validate passes, POST to http://127.0.0.1:4100/v1/projects/346e900f13e7b8d14798c54f/deploy with same body and auth
4. Report the live URL from the deploy response, or report any errors in detail.

## Your Tools (call these HTTP endpoints anytime)

### Get your current task
```bash
curl http://localhost:4100/v1/worker/worker-devops-b152ff04/task
```
...
### Report progress
```bash
curl -X POST http://localhost:4100/v1/worker/worker-devops-b152ff04/report -H 'Content-Type: application/json' -d '{
...
### Get your identity
```bash
curl http://localhost:4100/v1/worker/worker-devops-b152ff04/identity
```
...
- RESTART_EXIT_CODE = 75 — PM2/systemd/start-node.bat restarts the process when it exits with this code.
- MSYS2 path normalization: /c/Users/... is converted to C:\\Users\\... on Windows because path.join mishandles MSYS2 paths.
- EC2-2 extracts public key from iss peerId — no ledger lookup needed.
